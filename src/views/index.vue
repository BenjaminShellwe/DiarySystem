<template>
    <div>
        <page-header title="控制台页面">
            <template #content>
                <div>
                    <div style="margin-bottom: 5px;">小型开发,有错难免,多多原谅</div>
                </div>
            </template>
        </page-header>
        <page-main>
            <span>你是来 拉 取日记信息的吧</span>
        </page-main>
        <page-main>
            <el-row style="margin: 0 10px;">
                <el-col :span="12" class="Echarts">
                    ...
                    <span style="text-align: center; cursor: pointer;" @click="onIconClick('editor')"><h1>写日记</h1></span>
                </el-col>
                <el-col :span="12" class="Echarts">
                    <el-card id="chartsTable" shadow="hover" style="margin: 50px; padding: 25px; background-color: rgb(239, 239, 239);" />
                    <span style="text-align: center; cursor: pointer;" @click="onIconClick('gallery')"><h1>看(处刑)别人日记</h1></span>
                </el-col>
            </el-row>
        </page-main>
    </div>
...
export default {
    ...
    methods: {
        ...
        onIconClick(val) {
            router.push({path: '/multilevel_menu_basic/function_manage/' + val})
        },
